...
export class AssetsService {
  ...
  async findAll(householdId: string): Promise<AssetDocument[]> {
    return await this.assetModel.find({ householdId: new Types.ObjectId(householdId) }).exec();
  }
  ...
  async update(id: string, updates: any): Promise<AssetDocument | null> {
    return await this.assetModel.findByIdAndUpdate(id, updates, { new: true }).exec();
  }
